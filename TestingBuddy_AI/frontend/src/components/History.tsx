...
export default function History({ onBack }: Props) {
  ...
  async function handleDelete(record: any) {
    if (!window.confirm(`Delete test plan for ${record.project_key}? This will permanently remove the history entry and the generated .md and .docx files.`))
      return
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <div>
          <h2 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 700, color: 'var(--text-heading)' }}>Test Plan History</h2>
          <p style={{ margin: 0, fontSize: '0.82rem', color: 'var(--text-muted)' }}>Previously generated test plans — click to download</p>
        </div>
      </div>
